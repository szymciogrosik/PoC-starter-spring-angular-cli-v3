...
export class AuthService {
  ...
  private static removeUserFromStorage(): void {
    AuthService.removeDataFromStorage(Environment.LOGGED_USER);
  }

  private static removeTokenFromStorage(): void {
    AuthService.removeDataFromStorage(Environment.SECRET_USER_KEY);
  }
  ...
  public login(loginData: string) {
    return this.http.post<any>(Environment.BACKEND_URL + '/api/auth/signin', loginData)
      .pipe(map(res => {
        let user = new User();
  ...
  private addOrUpdateUserToStorage(user: User): void {
    AuthService.removeUserFromStorage();
    this.addDataToStorage(Environment.LOGGED_USER, user, Environment.SECRET_USER_KEY);
  }
  ...
  private getUserFromStorage(): User {
    return this.getDataFromStorage(Environment.LOGGED_USER, Environment.SECRET_USER_KEY);
  }
}
